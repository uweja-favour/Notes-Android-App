package com.xapps.notes.ui.theme
...
val goldenYellow = Color(0xFFF3B117)
val peach = Color(0xFFFFDAB9) // Light warm peach
val rosePink = Color(0xFFFF69B4) // Hot pink, slightly bold and trendy
val skyBlue = Color(0xFF87CEEB) // Bright sky blue
val lavender = Color(0xFFE6E6FA) // Gentle, light lavender
val aestheticHue = Color(0xFF9C88FF) // Purple-ish hue with character



val appPrimaryColor = Color(0xFF1367EC)
val appIconTint = Color.White
val appTertiaryColor = Color(0xFF876B8C)


val appContainerColor
    @Composable
    get() = when {
        !isSystemInDarkTheme() -> { // light theme
...
fun alarmCardContainerColor(alarmIsRinging: Boolean): Color {
    return when {
        !isSystemInDarkTheme() -> { // light theme
            if (alarmIsRinging) Color.Transparent else MaterialTheme.colorScheme.surfaceContainerLowest
        }

        else -> { // dark theme
            if (alarmIsRinging) Color.Transparent else MaterialTheme.colorScheme.surfaceContainer
        }
    }
}
